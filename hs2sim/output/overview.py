"""The per-run summary figures.

One function per figure, each reading only the results dictionary and the
propagated environment. The colours here are per-figure and predate
``style.py``: each is a standalone chart with its own fixed legend rather than
part of a shared encoding, so they are left as they were.
"""
...
def ground_track(cfg: MissionConfig, env: EnvironmentResult,
                 out_dir: pathlib.Path, plt) -> None:
    """Sub-satellite track with the ground station sites marked."""
    fig, ax = plt.subplots(figsize=(11, 5.5))
    r_fixed = np.einsum("nij,nj->ni", env.dcm_PN, env.r_BN_N)
...
def face_illumination(results: dict, out_dir: pathlib.Path, plt) -> None:
    """Time-averaged absorbed flux per body face, split by source."""
...
def array_trade(results: dict, out_dir: pathlib.Path, plt) -> None:
    """Peak and orbit-average output for each candidate array geometry."""
...
def temperature(cfg: MissionConfig, results: dict,
                out_dir: pathlib.Path, plt) -> None:
    """Single-node bulk temperature against the battery limits."""
    names = list(results["geometries"])
    if "temperature_series_c" not in results["geometries"][names[0]]:
        return
...
def payload_rate(results: dict, out_dir: pathlib.Path, plt) -> None:
    """Where the requested experiment cadence stops buying images."""
    fig, ax = plt.subplots(figsize=(7.5, 4.5))
    for name in results["geometries"]:
